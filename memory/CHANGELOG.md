...
- Lecture des vidéos longues attend l'aperçu optimisé (statut/retry par clip). La lecture forcée de l'original GOP10 peut encore manquer des frames sur cuts100ms ; ce chemin est désormais gardé avant disponibilité du proxy. Export reste frame-accurate sur original.
- Index keyframe binaire, budget frames+pending, flush EOF unique, dernière image, récupération logicielle persistante ; préchauffage du pool avant son, horizon mobile pendant plans longs, préparation annulable et pompe commune lecture/boucle/série ; scrub asynchrone fiabilisé.
- Vérifié : 6 tests HTTP/médias + 2 tests unitaires retry ; Chrome réel /studio iframe : 690 ticks proxy desktop et 595 mobile simulé sans frame manquante ou mauvaise ; MP4 réel1,2s/36frames/1920×1080 avec audio serveur. Source185s/4440frames. Firefox/Safari physique non testés.
- Harnais frontend précédents avaient des erreurs de portée lexicale (`window.clips` != `clips`) et de timecode (source vs montage). Harnais final corrigé : `frontend/tests/test_long_preview_regression_iter34.py`, résultat `test_reports/iteration_35.json`. Ne pas modifier le produit pour contourner ces anciens tests invalides.
- PRD séparé en document actuel, cet historique et ROADMAP. Aucun changement d'authentification, de paiement, de clés ou de credentials.

---

Les entrées suivantes sont historiques ; consulter PRD.md et ROADMAP.md pour l'état actuel.
