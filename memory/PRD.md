...
## 2026-06 — Firefox Windows : cause trouvée via télémétrie prod, correctif « bitmap mode » (v13.31-ff-bitmap)
- Données brutes prod (Firefox 155 Windows, TADC 720p60 proxy) : `preview_stall` avec `queue:15, pending:18, buffered:0, soft:false` répétés + `low_fps 15` → le décodeur matériel WMF cesse de sortir des images par à-coups (~500 ms) alors que la file d'entrée est pleine. Signature connue Windows (SO 79686288) : le décodeur HW bloque quand des VideoFrame décodées restent ouvertes (limite de surfaces), la libération étant paresseuse.
- Fix : `WC_BITMAP_MODE` (Firefox uniquement) — dans `SegPlayer._onFrame`, chaque image est copiée via `createImageBitmap(f)` puis `f.close()` immédiat ; le bitmap reçoit `timestamp/duration/displayWidth…` en expando et remplace la VideoFrame partout (drawImage, cutCache, thumbs, hook, export canvas). `pending` décrémenté à l'arrivée du bitmap ; bitmaps tardifs (génération changée) fermés. Caps réduits en bitmap mode (8 actif / 3 idle). Anomalie ts non remontée en bitmap mode (ordre async).
- Vérifié (Firefox Linux réel, vidéos réelles du user, 52 cuts et plan unique) : 0 gel, 60 fps, `cur instanceof ImageBitmap` true, export MP4 OK (`hybrid`, 19 frames/1.2 s). Chrome : mode inactif, 0 gel.
- Non vérifiable ici : le WMF Windows lui-même → attendre le retour du user après déploiement ; relire `admin/telemetry/preview/raw?browser=Firefox`.

## 2026-06 — Clés API front (audit), bandeau cookies, favicon
- Audit clés API front : aucune clé dans `frontend/src`, `public/*.js|html`, `.env` front (seulement REACT_APP_BACKEND_URL) ni dans le bundle prod `main.*.js` (regex sk_/pk_/re_/gsk_/r8_/AIza/whsec_). Toutes les clés (Stripe, Groq, Pexels, Resend, Replicate, Mux) sont côté `backend/.env` uniquement, appelées via proxies `/api/*`. Rien à retirer.
- Bandeau cookies : `src/components/CookieBanner.js` (React, monté dans App.js sous AppRouter) — FR/EN via useI18n, style v3 (noir, filet, carré crimson, boutons pill), mobile plein largeur bas / desktop carte 420 px bas-droite, safe-area, lien /confidentialite. Choix stocké `localStorage.bc_cookie_consent = {v:'accepted'|'refused',at}`. Fallback vanilla dans `studio.html` (fin de body) affiché seulement si `window.top===window` (accès direct), même clé → pas de doublon dans l'iframe. Testids : cookie-banner, cookie-accept-btn, cookie-refuse-btn, cookie-more-link.
- Favicon : logo « B. » du user (asset 4.png 2000 px, recadré 60 %) → `public/favicon.ico` (16-64), `favicon-16/32/192/512.png`, `apple-touch-icon.png`, `manifest.json` ; liens dans `index.html` (%PUBLIC_URL%) et `studio.html`. Le dev server a nécessité un restart frontend pour recharger index.html.
- Vérifié par captures : landing desktop 1920 + mobile 390 (bandeau), accept → disparaît et persiste au reload, studio direct (bandeau vanilla) vs studio dans l'app (uniquement bandeau React), favicon.ico 200 image/x-icon.

## 2026-06 — « Ajouter à l'écran d'accueil » (PWA légère)
- `src/components/AddToHomeScreen.js` monté dans App.js : iPhone Safari (UA iPhone/iPad, hors CriOS/FxiOS, hors standalone) → carte v3 avec logo + étapes « Partager → Sur l'écran d'accueil » ; Android/Chrome → capte `beforeinstallprompt` et bouton « Installer ». Affiché seulement sur /dashboard, /projects, /studio, après consentement cookies et sans bandeau cookies présent ; « Plus tard » / ✕ = snooze 14 j (`localStorage.bc_a2hs_snooze`). Testids : a2hs-banner, a2hs-later-btn, a2hs-install-btn, a2hs-close-btn, a2hs-ios-steps.
- `index.html` : meta `apple-mobile-web-app-capable`, `mobile-web-app-capable`, `apple-mobile-web-app-title=BEATCUT`, status bar black-translucent ; `manifest.json` display standalone + icônes 192/512 (déjà). Note iOS : l'app écran d'accueil a son propre stockage → une reconnexion est nécessaire la 1re fois.
- Vérifié (Chrome 390 px avec UA iPhone via add_init_script) : bandeau visible après login, plus de chevauchement avec cookies, « Plus tard » ferme et persiste au reload.
